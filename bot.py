...
def main_handler(message):
    ...
    if text == BTN_REGISTER:
        ...
        msg = bot.send_message(chat_id, "Введите имя получателя:")
        bot.register_next_step_handler(msg, delivery_step_name)

    elif text == BTN_ADDRESS:
        msg = bot.send_message(chat_id, "Введите имя (только латиница):")
...
def address_step_name(message):
    ...
    if not message.text.isascii():
        msg = bot.send_message(chat_id, "❌ Только латиница!")
        bot.register_next_step_handler(msg, address_step_name)
        return
    user_data[chat_id] = {"name": message.text}
    msg = bot.send_message(chat_id, "Введите телефон:")
...
def update_track_cache_periodically():
    while True:
        try:
            load_cache()
            print(f"[INFO] Кэш обновлён, треков: {len(track_cache)}")
        except Exception as e:
